...
object Player extends App {
  ...
  def connect(nodes: Seq[Seq[Node]]) = {
  ...
  val nodes = for(y <- 0 until height) yield readLine().zipWithIndex.flatMap {case (v, x) => if(v.equals('0')) Some(Node(x, y, None, None)) else None}

  val grid = connect(nodes)
